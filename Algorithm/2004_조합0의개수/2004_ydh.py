...
def count2(N): # 입력값의 소인수 분해 중 2의 개수 카운트
    ...
    count = 0
    while N >= 2: 
        count += N//2 # 입력값을 2로 나눈 몫을 반복하여 더하기
        N = N//2 # 2의 배수인 경우 ex) 4,8,16 반복
    return count
...
def count5(N): # 입력값의 소인수 분해 중 5의 개수 카운트
    ...
    count = 0
    while N >= 5: # 입력값을 5로 나눈 몫을 반복하여 더하기
        count += N//5 # 입력값을 5로 나눈 몫을 반복하여 더하기
        N //= 5 # 5의 배수인 경우 ex) 5,25,125 반복
    return count
